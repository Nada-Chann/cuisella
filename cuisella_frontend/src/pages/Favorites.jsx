...
const Favorites = () => {
  ...
  if (loading)
    return (
      <p className="pt-20 text-center text-[#4B352A] font-semibold">
        Loading favorites...
      </p>
    )
  if (error)
    return (
      <p className="pt-20 text-center text-red-500 font-semibold">{error}</p>
    )
  ...
  return (
    <div className="bg-[#EFE4D2] min-h-screen px-4 py-8 mt-16">
      ...
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-serif font-bold text-[#CA7842] mb-10 text-center">
          Your Favorite Recipes
        </h1>

        {favorites.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-[#4B352A] mb-4 font-semibold">
              You haven't added any favorite recipes yet.
            </p>
            <Link
              to="/recipes"
              className="text-[#CA7842] hover:text-[#954C2E] font-medium"
            >
              Explore Recipes →
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-10">
            {favorites.map(({ id, recipe }) => (
              <div
                key={id}
                className="relative bg-[#F1EFEC] rounded-3xl shadow-md pt-24 px-5 pb-5 flex flex-col"
              >
                {recipe.image_path && (
                  <img
                ...
                <div className="px-5 pb-5 flex flex-col flex-grow">
                  <h3 className="text-xl font-semibold text-[#4B352A] mb-2 text-center">
                    {recipe.title}
                  </h3>
                  <p className="text-[#4B352A] mb-4 line-clamp-2 text-sm">
                    {recipe.description || "No description available."}
                  </p>

                  <div className="flex justify-between items-center text-sm text-[#4B352A] mb-4">
                    <span>
                      {/* Optionally add time and servings if available */}
                      {recipe.time && (
                        <>
                  ...
                  <div className="mt-auto flex justify-center">
                    <Link
                      ...
                      className="text-[#CA7842] hover:text-[#954C2E] flex items-center gap-1"
                      title="View Recipe"
                    >
